...
//The parameter "livery" is provided by the server as a string, either a HTTPS url, number, or several URLs joined together by "|"
function changeLivery(livery) {
	if (debug) console.log("Livery Change Request as " + "'" + livery + "'");
	if (livery === "inv") { //If the custom livery client check fails
	...
	if (livery.toString().includes("https://")) {
		if (geofs.aircraft.instance.id == 4140) { //737-200 uses different indices in the GLTF file than most aircraft.
			if (livery.toString.includes("|")) { //server sends several map urls split by "|" for PBR aircraft
				var sL = livery.split("|");
				var composite = sL[1]; //composite (gltfMetallicRoughness) map index
				var normal = sL[2]; //openGL normal map index
		...
		geofs.aircraft.instance.loadLivery(livery); //GeoFS also supports self-hosted liveries for each aircraft.
		if (debug) console.log("livery changed to " + livery);
	}
}

window.addEventListener('message', function(e) { //listens for livery change requests from the menu window.
	if (debug) console.log("Event Recieved: " + e);
	if (e.origin !== "https://boeingbus747.github.io/new/main/ML/UIbyAriakimTaiyo/index.html") return;
	changeLivery(e.data);
}, false);
...
buttonDiv.addEventListener("click", function() {
	...
	mlui = window.open("https://boeingbus747.github.io/new/main/ML/UIbyAriakimTaiyo/index.html", "_blank", "height=1000,width=1500");
	opened = true;
...
if (document.getElementsByClassName("fmc-btn").length == 0) { //The FMC addon intrudes on the button space, so we need to move it a bit.
...
function setupMP() {
	if (geofs.version == 2.9) { //legacy compatability.
		...
		updateMultiplayer = function() {
			Object.values(multiplayer.visibleUsers).forEach(function(e) { //Iterate through all multiplayer user models and apply custom liveries, if present.
				a = e;
				livObj.aircraft.forEach(function(e) {
					if (a.currentLivery == parseInt(e.livery)) {
